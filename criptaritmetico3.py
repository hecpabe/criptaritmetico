...
#   6: Checking combinations with zero (DEFAULT FALSE)
# Return: True if chases the result false if not
def getCombinations(wordsList, uniqueCharactersList, charactersCodes, fixedCodes, level, n, checkingZeros=False):
    ...
    if (level == n):
        ...
        # Check if the correct (or not) solution could match with zeros
        if (not checkingZeros):
            for i in range(0, len(charactersCodes)):
                for key, value in charactersDictionary.items():
                    if (charactersCodes[i] == value):
                        if (key != wordsList[0][0] and key != wordsList[1][0] and key != wordsList[2][0]):
                            charactersCodesCopy = charactersCodes.copy()
                            charactersCodesCopy[i] = 0
                            getCombinations(wordsList, uniqueCharactersList, charactersCodesCopy, fixedCodes, level, n, True)

    # Keep generating permutations by backtracking
    else:
        for i in range(level, n):
            # 1.- Permutate values
            # 2.- Check permutated values
            # 3.- Undo the permutation (backtrack)
...
# Parameters:
#   0: Unique characters array
#   1: Fixed characters dictionary
